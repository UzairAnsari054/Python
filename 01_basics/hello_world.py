...
# mtln aap varible ko kabhi datatype assign nhi krte par memory me jo value store hoti h as a reference to that variable usse datatype assign hota h aur yeh datatype meory me hi rehta h apni value ke saath, varaible se datatype ka koi lena dena nhi h
...
# Ab hame pata h ki, agr memory me stored value ko bahare se koi point nhi krra toh garbage collector aaega aur uss value ko delete krdega, auresa hi hota h par 1 exception h ki NUMBERS AUR STRINGS ke cases me yeh garbage collection immediately nhi hota, forcefully karwa skte ho, pr immediately nhi hota. (Kyun ke maybe same number ka refrence program ko next step me chahiye ho...``)
...
# So, each object in the memory can be refrenced by multiple variables. Also, we can get the count of how many variables are reffrecing a specific object, the count is known as referenceaCount/refCount. But  sys.getrefcount() gives the same count for every string or integer
